...
function updateBall(ball) {
  ...
  ball.x += ball.vx * state.frameStep;
  ball.y += ball.vy * state.frameStep;
...
function updateLasers() {
  state.lasers = state.lasers.filter((laser) => {
    laser.y += laser.vy * state.frameStep;
    for (const brick of state.bricks) {
      if (brick.hp <= 0) continue;
...
function updatePowerUps() {
  state.powerUps = state.powerUps.filter((powerUp) => {
    powerUp.y += powerUp.vy * state.frameStep;
...
function updateParticles() {
  state.particles = state.particles.filter((particle) => {
    particle.x += particle.vx * state.frameStep;
    particle.y += particle.vy * state.frameStep;
    particle.vy += 0.04 * state.frameStep;
    particle.life -= state.frameStep;
...
function update(frameStep = 1) {
  state.frameStep = frameStep;

  if (band.heldDirection !== 0 && performance.now() < band.heldUntil) {
    nudgePaddle(band.heldDirection, 9 * frameStep);
  }

  const smoothing = 1 - Math.pow(1 - CONTROL_SMOOTHING, frameStep);
  state.paddle.x += (state.paddle.targetX - state.paddle.x) * smoothing;

  if (state.paddle.wideTimer > 0) {
    state.paddle.wideTimer -= frameStep;

    if (state.paddle.wideTimer <= 0) {
      state.paddle.wideTimer = 0;
      state.paddle.w = 96;
      state.paddle.targetX = clamp(state.paddle.targetX, 6, WIDTH - state.paddle.w - 6);
      setMessage("Paddle normal.");
    }
  }

  if (state.paddle.laserTimer > 0) state.paddle.laserTimer -= frameStep;
  if (state.pierceTimer > 0) state.pierceTimer -= frameStep;
